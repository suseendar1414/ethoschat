...
#             schemas[table] = execute_sql_query(query).to_dict()
#         return schemas

#     # Initialize chat messages history
#     if "messages" not in st.session_state:
#         st.session_state.messages = [{"role": "assistant", "content": "How can I help you with your Snowflake data?"}]
...
#                     model="gpt-3.5-turbo",
#                     messages=[
#                         {"role": "system", "content": f"You are a SQL expert. Generate a SQL query based on the user's question. Use only the tables and columns provided in the schema information. Here are the table schemas: {json.dumps(table_schemas)}"},
...
#                     result_json = query_result.to_json(orient='records')
#                 except Exception as e:
#                     result_json = f"Error executing query: {str(e)}"
                
#                 # Summarize results
#                 final_response = openai_client.chat.completions.create(
#                     model="gpt-3.5-turbo",
#                     messages=[
#                         {"role": "system", "content": "You are a data analyst. Summarize the query results in a concise and informative way, answering the user's original question."},
...
#                 # Optionally display SQL query and results
#                 with st.expander("See SQL query and results"):
#                     st.subheader("Generated SQL Query")
#                     st.code(sql_query, language="sql")
#                     st.subheader("Query Results")
#                     st.dataframe(query_result if 'query_result' in locals() else "No results to display")

#         # Add assistant response to chat history
#         st.session_state.messages.append({"role": "assistant", "content": response})

# else:
#     st.warning("Please connect to Snowflake using the sidebar to start chatting.")
